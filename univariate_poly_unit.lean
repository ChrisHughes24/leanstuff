...
@[simp] lemma mod_by_monic_C_eq_zero (a : α) (hq : monic q) : mod_by_monic (C a) hq = 0 :=
begin
...
lemma degree_le_of_euclid_size_poly (h : euclid_size_poly p ≤ euclid_size_poly q) :
  degree p ≤ degree q :=
begin
  unfold euclid_size_poly at h,
  split_ifs at h,
...
lemma euclid_size_poly_lt_of_degree_lt (h : degree p < degree q) :
  euclid_size_poly p < euclid_size_poly q :=
begin
...
if h : 0 < degree q then
euclid_size_poly_lt_of_degree_lt $ begin 
  unfold mod_aux,
  rw dif_neg hq,
  have := degree_mod_by_monic_lt p (monic_mul_leading_coeff_inv hq),
  rw degree_mul_leading_coeff_inv hq at this,
  exact this h,
end
else begin
...
if h : 0 = degree q then begin 
  unfold mod_aux,
  rw eq_C_of_degree_eq_zero h.symm at *,
  rw dif_neg hq,
  unfold mod_by_monic div_mod_by_monic_aux,
  by_cases hp : (0 < degree p),
  simp [hp, nat.zero_le],
end
