...
lemma nat.prime.ne_one {p : ℕ} (hp : p.prime) : p ≠ 1 :=
...
@[simp] lemma rat.neg_num : ∀ q : ℚ, (-q).num = -q.num
| ⟨(n : ℕ), _, _, _⟩ := rfl
| ⟨-[1+ n], _, _, _⟩ := rfl

lemma nat.one_lt_iff_ne_zero_and_ne_one : ∀ {n : ℕ}, 1 < n ↔ n ≠ 0 ∧ n ≠ 1
| 0     := dec_trivial
| 1     := dec_trivial
| (n+2) := dec_trivial

@[simp] lemma rat.one_num : (1 : ℚ).num = 1 := rfl

@[simp] lemma rat.one_denom : (1 : ℚ).denom = 1 := rfl

lemma rat.add_num_denom (q r : ℚ) : q + r =
...
by conv_lhs { rw [rat.num_denom q, rat.num_denom r, rat.add_def hqd hrd] };
  simp [mul_comm]
