import group_theory.perm group_theory.order_of_element
...
lemma pow_apply_eq_self_of_apply_eq_self_nat {f : perm α} {x : α} (n : ℕ) (hfx : f x = x) :
  (f ^ n) x = x :=
by induction n; [refl, rw [pow_succ, mul_apply, n_ih, hfx]]

def support_equiv_gpowers [fintype α] {f : perm α} (hf : is_cycle f) : gpowers f ≃ {x | x ∈ f.support} :=
let ⟨x, hx⟩ := classical.indefinite_description _ hf in
{ to_fun := λ g, let i := classical.some g.2 in ⟨(f ^ i) x,
    mem_support.2 (mt (injective.eq_iff (f ^ -i).bijective.1).2 $
      begin
        rw [← mul_apply, ← mul_apply, ← mul_apply, ← gpow_add, neg_add_self, gpow_zero];
        rw [← gpow_one f] {occs := occurrences.pos [2]},
        rw [← gpow_add, ← gpow_add, neg_add_eq_sub, sub_add_cancel, gpow_one],
        exact hx.1
      end)⟩,
  inv_fun := λ x, begin end  }

lemma order_of_eq_card_support [fintype α] (f : perm α) : order_of f = f.support.card :=
begin
  rw [order_eq_card_gpowers, ← set.card_fintype_of_finset f.support (@mem_support _ _ _ f)],
  refine @fintype.card_congr _ _ (id _) (id _)
  { to_fun := λ ⟨g, h⟩,
    inv_fun := _,
    left_inv := _,
    right_inv := _  },
