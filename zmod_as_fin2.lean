...
have h : nat_abs (a % n) < n := int.coe_nat_lt.1 begin 
  ...
  exact int.mod_lt _ (int.coe_nat_ne_zero_of_pos h0.pos)
end,
int.coe_nat_inj $ 
 by conv {to_lhs, rw [← cast_int_mod n a, 
    ← nat_abs_of_nonneg (mod_nonneg _ (int.coe_nat_ne_zero_of_pos h0.pos)),
...
⟨λ h, by have := fin.veq_of_eq h;
  rwa [cast_val_int, cast_val_int, ← int.coe_nat_eq_coe_nat_iff,
    ...
    nat_abs_of_nonneg (int.mod_nonneg _ (int.coe_nat_ne_zero_of_pos h0.pos))] at this,
λ h : a % n = b % n, 
