...
def conjugacy_classes_A5_meta_aux : list (alternating (fin 5)) → multiset
  (Σ a : alternating (fin 5), multiset
  {b : alternating (fin 5) × alternating (fin 5) // b.2 * a * b.2⁻¹ = b.1})
| [] := 0
| (a :: l) := let m : Σ a : alternating (fin 5), multiset
    {b : alternating (fin 5) × alternating (fin 5) // b.2 * a * b.2⁻¹ = b.1} :=
  ⟨a, quotient.lift_on (@univ (alternating (fin 5)) _).1
  (λ l, (l.map
  (λ x, show {b : alternating (fin 5) × alternating (fin 5) // b.2 * a * b.2⁻¹ = b.1},
    from ⟨(x * a * x⁻¹, x), rfl⟩)).pw_filter (λ x y, x.1.1 ≠ y.1.1)) sorry⟩ in
have wf : (l.filter (λ x, x ∉ m.2.map (prod.fst ∘ subtype.val))).length <
    (a :: l : list _).length,
  from  sorry,
m :: conjugacy_classes_A5_meta_aux (l.filter (λ x, x ∉ m.2.map (prod.fst ∘ subtype.val)))
using_well_founded {rel_tac := λ _ _, `[exact ⟨_, measure_wf list.length⟩],
  dec_tac := tactic.assumption}

def conjugacy_classes_A52 : list A5 → 
  multiset {s : finset A5 //  ∀ (x∈s) (y∈s), is_conj x y}
| []     := ∅
| (a::l) := 
  let s : {s : finset A5 // ∀ (x∈s) (y∈s), is_conj x y} :=
  ⟨(@univ A5 _).image (λ x, x * a * x⁻¹), 
    λ x hx y hy, let ⟨m, _, hm⟩ := mem_image.1 hx in
      let ⟨n, _, hn⟩ := mem_image.1 hy in 
      ⟨n * m⁻¹, by simp [hm.symm, mul_assoc, hn.symm]⟩⟩ in
  have wf : false, from sorry,
  s :: conjugacy_classes_A52 (l.filter (λ x, x ∉ s.1))
using_well_founded {rel_tac := λ _ _, `[exact ⟨_, empty_wf⟩],
  dec_tac := tactic.assumption}

-- def conjugacy_classes_A5_meta : multiset (Σ a : alternating (fin 5), multiset
--   {b : alternating (fin 5) × alternating (fin 5) // b.2 * a * b.2⁻¹ = b.1}) :=
...
def gcd' : nat → nat → nat
| 0        y := y
| (succ x) y := have false, from sorry,
                gcd (y % succ x) (succ x)
using_well_founded {rel_tac := λ _ _, `[exact ⟨_, empty_wf⟩],
  dec_tac := tactic.assumption}

example : gcd' 1 2 = 1 := rfl
...
@[irreducible] def conjugacy_classes_A5_aux : multiset (Σ a : alternating (fin 5), multiset
  {b : alternating (fin 5) × alternating (fin 5) // b.2 * a * b.2⁻¹ = b.1}) :=
by exact_reflect (conjugacy_classes_A5_meta_aux (quot.unquot univ.1))

example : conjugacy_classes_A5_aux = conjugacy_classes_A5_aux := dec_trivial

def conjugacy_classes_A5_aux2 : multiset (multiset (alternating (fin 5))) :=
conjugacy_classes_A5_aux.map (λ s, s.2.map (λ b, b.1.1))

example : (quotient.lift_on (@univ A5 _).1 conjugacy_classes_A52 sorry).map (finset.card ∘ subtype.val) =
  {1,12,12,15,20} := dec_trivial
