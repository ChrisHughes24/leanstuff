import data.matrix data.rat linear_algebra.basis data.fintype tactic.omega algebra.associated
...
@[simp] lemma finset.length_sort (s : finset (fin n)) : (s.sort (≤)).length = s.card := sorry
...
-- λ i j, det (minor M
--   (λ i' : fin (n - 1), if i'.1 < i.1 then i'.cast_le sorry
--     else i'.succ) _)
...
--linear_independent ℚ (↑(B.image (column A)) : set (cvec m))
...
#eval (is_feasible ex.A ex.b ex.x : bool)

--set_option trace.class_instances true
#eval (check_reduced_cost ex.A ex.c ex.x ex.B : bool)
