import tactic.find data.nat.prime algebra.group_power tactic.ring data.set.finite .disjoint_finset data.int.modeq
universes u v
...
lemma int.coe_nat_div (a b : ℕ) : ((a / b : ℕ) : ℤ) = (a : ℤ) / b := rfl
...
lemma mul_self_mod_injective_of_le_half_prime {p : ℕ} (hp : prime p) (hpo : p % 2 = 1) : function.injective
  (λ a : fin (succ (p / 2)), (a.1 : ℕ) * a.1 % p) := λ a b (h : a.1 * a.1 ≡ b.1 * b.1 [MOD p]),
begin
  wlog h₁ : a.1 ≤ b.1 using a b,
