import algebra.group_power algebra.big_operators data.nat.choose data.fintype
local infix ` ^ ` := monoid.pow
open finset nat fintype
variable {α : Type*}
...
$ λ n hi h, begin 
  ...
  { rw [sum_range_succ e, add_comm (e _), nat.add_sub_assoc hm, _root_.pow_add],
    simp only [mul_assoc, mul_left_comm ((λ i, f i * r i) ∑ succ n ^ e ∑ succ n)],
...
def fun_fin_to_nat (n : ℕ) (f : fin n → α) [inhabited α] : ℕ → α := 
        λ i, if h : i < n then f ⟨i, h⟩ else arbitrary α

lemma finset.sum_def {β : Type*} [add_comm_monoid β] (s : finset α) (f : α → β) : finset.sum s f = (s.1.map f).sum := rfl
#print multiset.map
theorem missing2 [comm_semiring α] (n : ℕ) (f : fin n → α) (e : fin n → ℕ) (r : fin n → α)
    (s : α) : (∀ i : fin n, (f i) ^ (e i) * s = 0) → 
    sum univ (λ i : fin n, f i * r i) = 1 → s = 0 := λ h₁,
begin
  have := missing1 n (λ i, dite (i < n) (λ h, f ⟨i, h⟩) (λ h, 0)) (λ i, dite (i < n) (λ h, e ⟨i, h⟩) (λ h, 0))
  (λ i, dite (i < n) (λ h, r ⟨i, h⟩) (λ h, 0)) s (λ i hi, by simp [hi, h₁]),
  have : (λ i, (λ (j : ℕ), dite (j < n) (λ (h : j < n), f ⟨j, h⟩) (λ (h : ¬j < n), 0)) i *
           (λ (j : ℕ), dite (j < n) (λ (h : j < n), r ⟨j, h⟩) (λ (h : ¬j < n), 0)) i) ∑ n = 
        finset.sum univ (λ i, f i * r i) := begin rw [finset.sum_def, finset.sum_def],
        refine congr rfl _,
        simp,
         end,
end
